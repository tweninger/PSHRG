...
class Rule(object):
    def __init__(self, id, lhs, s_rhs, prob, translate=True):
        self.id = id
        self.lhs = lhs
        if translate:
            self.rhs = s_rhs
            self.cfg_s_rhs = self.hrg_to_s_cfg(lhs, s_rhs)
        else:
            self.cfg_s_rhs = s_rhs
        self.prob = prob
        if len(self.cfg_s_rhs) == 0:
            print "T"

    def hrg_to_s_cfg(self, lhs, s_rhs):
        s_symb = []
        for rhs in s_rhs:
            t_symb = set()
            n_symb = []
            ...
            s_symb.append( list(t_symb) + n_symb )
...
class Grammar(object):
    ...
    def set_max_size(g, max_size, rhs_side=0):
        unary_graph = nx.DiGraph()
        for x in g.nonterminals:
            unary_graph.add_node(x)
        for lhs, rules in g.by_lhs.items():
            for rule in rules:
                if len(rule.cfg_s_rhs[rhs_side]) == 1 and rule.cfg_s_rhs[rhs_side][0] in g.nonterminals:
                    unary_graph.add_edge(rule.lhs, rule.cfg_s_rhs[rhs_side][0], weight=rule.prob)
        ...
        for size in range(1, max_size + 1):
            ...
            for lhs_i, lhs in enumerate(topological):
                for rule in g.by_lhs[lhs]:
                    ...
                    nts = [nt_to_index[x] for x in rule.cfg_s_rhs[rhs_side] if x in g.nonterminals]
                    n = size - (len(rule.cfg_s_rhs[rhs_side]) - len(nts))  # total size available for nonterminals
        ...
        while i < len(w):
            if w[i][0] in g.nonterminals:
                ...
                for rule in g.by_lhs[lhs]:
                    ...
                    nts = [nt_to_index[x] for x in rule.cfg_s_rhs[rhs_side] if x in g.nonterminals]
                    n = lhs_size - (len(rule.cfg_s_rhs[rhs_side]) - len(nts))  # total size available for nonterminals
                ...
                cfg_rhs = []
                j = 0
                for y in rule.cfg_s_rhs[rhs_side]:
