...
def nx_to_graph(g):
    ...
    for v in g.nodes():
        nodes[str(v)] = Node('u')
        nodes[str(v)].var = str(v)
    for u, v in g.edges():
        label = g[u][v].get('label', 'E')
        nodes[str(u)].outedges.append(Edge(label, [nodes[str(v)]]))

    max = -1
    max_node = None
    for n in nodes:
        s = len(nodes[n].outedges)
        if s > max:
            max = s
            max_node = nodes[n]
    root = max_node
    ...
    for n in g.dfs():
        scc.add(n.var)
    others_str = set(nodes.keys()).difference(scc)
    others = []
    for n in others_str:
        others.append( nodes[n] )
    if len(others) > 0:
        ge = graph_parser.graph.Edge(label='F', tails=list(others))
