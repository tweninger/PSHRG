...
def parse(g, starts, h):
    ...
    for rule in g:
        if verbose >= 3:
            print("Rule {}: {} -> {}".format(rule.id, rule.lhs, format_rhs(rule.rhs, show_all=True)))
...
if __name__ == "__main__":
    ...
    argparser.add_argument('grammars', type=str, metavar='grammar', nargs='+', help='file(s) containing grammar rules')
    argparser.add_argument('-s', '--start', dest='start', type=str, default='Truth', metavar='nonterminal',
                        help='start symbol')
    ...
    for grammarfile in args.grammars:
        for ri, line in enumerate(open(grammarfile)):
